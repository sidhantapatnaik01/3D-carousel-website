...
// ── WATERMARK MASK POSITIONING ────────────────────────────
// The KlingAI watermark is baked into every frame at a fixed relative position.
// We mirror the drawFrame() contain-scale math to find exactly where it
// lands in viewport coordinates, then move #wm-mask there.
(function initWmMask() {
  const wmMask = document.getElementById('wm-mask');
  if (!wmMask) return;

  // Relative position of the KlingAI watermark within the source frame
  // (estimated from screenshots — right ~12 %, bottom ~4 % of frame size)
  const WM_FROM_RIGHT_PCT  = 0.12;
  const WM_FROM_BOTTOM_PCT = 0.04;
  const WM_W_PCT           = 0.20; // width  as fraction of drawn image width
  const WM_H_PCT           = 0.09; // height as fraction of drawn image height

  function positionMask() {
    const ref = frames.find(f => f.complete && f.naturalWidth);
    if (!ref) return;

    const cw = window.innerWidth;
    const ch = window.innerHeight;
    const scale = Math.min(cw / ref.naturalWidth, ch / ref.naturalHeight);
    const dw = ref.naturalWidth  * scale;
    const dh = ref.naturalHeight * scale;
    const dx = (cw - dw) / 2; // left edge of drawn image in viewport px
    const dy = (ch - dh) / 2; // top  edge of drawn image in viewport px

    // Watermark bounding box inside drawn image
    const wmW = dw * WM_W_PCT;
    const wmH = dh * WM_H_PCT;
    // Distance from viewport edges
    const fromRight  = (cw - (dx + dw)) + dw * WM_FROM_RIGHT_PCT;
    const fromBottom = (ch - (dy + dh)) + dh * WM_FROM_BOTTOM_PCT;

    wmMask.style.position = 'fixed';
    wmMask.style.right    = `${Math.max(0, fromRight)}px`;
    wmMask.style.bottom   = `${Math.max(0, fromBottom)}px`;
    wmMask.style.width    = `${wmW}px`;
    wmMask.style.height   = `${wmH}px`;
    wmMask.style.left     = '';    // clear any mobile CSS overrides
    wmMask.style.background = '#070707';
    wmMask.style.zIndex   = '9998';
    wmMask.style.pointerEvents = 'none';
  }

  // Run once a frame has loaded, then on every resize
  let positioned = false;
  frames.forEach(img => {
    img.addEventListener('load', () => {
      if (!positioned) { positionMask(); positioned = true; }
    });
  });
  window.addEventListener('resize', positionMask);
  // Immediate attempt in case frames are cached
  positionMask();
})();
